...
$(document).ready(function () {
  var table_hs = $('#total_honorarios').DataTable({
    responsive: true,
    searching: true,
    paging: true,
    language: {
    ...
    buttons: [
      {
        ...
        filename: 'Honorarios_Registrados',
        orientation: 'landscape',
        className: 'btn btn-dexcel btn-success',
        exportOptions: {
          columns: [0, 1, 2, 3, 4, 5, 6],
  ...
  table_hs.buttons(['print']).remove();

  table_hs.buttons().nodes().each(function (node) {
    $(node).removeClass('btn-default').addClass('btn-sm');
  });

  $('#total_honorarios').on('buttons.export', function (e, button, config) {
    var updatedData = table_hs.buttons.exportData(config);
    config.data = updatedData;
  });
