...
if(isset($_POST['nameBuscaRut'])) {
    ...
    $queryTrabajador = "SELECT IDTra, Rut FROM trabajador WHERE Rut = '$rut'";
    $resultTrabajador = mysqli_query($conn, $queryTrabajador);

    $queryHonorario = "SELECT IDTraH, Rut FROM honorario WHERE Rut = '$rut'";
    $resultHonorario = mysqli_query($conn, $queryHonorario);

    // Redireccionar a resultados.php con los resultados de búsqueda
    header("Location: resultados.php?rut=$rut");
    var_dump(mysqli_num_rows($resultTrabajador));
var_dump(mysqli_num_rows($resultHonorario));
...
<script>
    document.getElementById("searchForm").addEventListener("submit", function(e) {
        ...
        if (input.length < 9) {
            ...
            // Redireccionar a resultados.php con los resultados de búsqueda
            window.location.href = "resultados.php?rut=" + input;
        }
    });
