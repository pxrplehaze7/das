...
// $pdfapelo = (!empty($_FILES['nameApelacionEDIT']['name'])) ? str_replace(array(' ', '(', ')'), '_', $_FILES['nameApelacionEDiT']['name']) : '';
...
if (mysqli_num_rows(mysqli_query($conn, "SELECT * FROM calificaciones WHERE IDCalif = '$idcalif'")) > 0) {
    ...
    //     move_uploaded_file($_FILES['nameApelacionEDIT']['tmp_name'], $ruta_ApelaFINAL);
